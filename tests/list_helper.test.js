...
describe('most likes', () => {
  ...
  test('of a bigger list is calculated right, identifying the author with the most likes', () => {
  ...
  test('of a list where multiple authors have the same highest number of likes returns one of them', () => {
    const blogsTiedLikes = [
      {
        _id: '5a422a851b54a676234d17f7',
        title: 'React patterns',
        author: 'Michael Chan',
    ...
    const result = listHelper.mostLikes(blogsTiedLikes);
    expect(result.likes).toBe(10);
    // Since it's a tie, the test could either expect 'Michael Chan' or 'Robert C. Martin'
    expect(['Michael Chan', 'Robert C. Martin']).toContain(result.author);
  });
});
